...
from components.DeepLab_V3plus import dense_prediction_cell
from components.DeepLab_V3plus import feature_extractor
from components.DeepLab_V3plus import utils
...
def multi_scale_logits(images,
  ...
  for output in sorted(model_options.outputs_to_num_classes):
    ...
    merge_fn = (
        tf.reduce_max
        if merge_method == 'max' else tf.reduce_mean)
  ...
  if not model_options.aspp_with_batch_norm:
    ...
    if model_options.dense_prediction_cell_config is not None:
      ...
      concat_logits = dense_prediction_layer.build_cell(
          ...
          image_pooling_crop_size=in_image_pooling_crop_size,
          weight_decay=weight_decay,
          reuse=reuse,
          is_training=is_training,
          fine_tune_batch_norm=fine_tune_batch_norm)
          ...
          if model_options.add_image_level_feature:
            if model_options.crop_size is not None:
              ...
              image_feature = tf.reduce_mean(
                  features, axis=[1, 2], keep_dims=True)
...
def _get_logits(images,
  ...
  features, end_points = extract_features(
      images,
      model_options,
      in_image_pooling_crop_size=image_pooling_crop_size,
      multi_grid=multi_grid,
      weight_decay=weight_decay,
      reuse=reuse,
      is_training=is_training,
