...
from components.NasNet import nasnet_utils
...
def large_imagenet_config(data_format):
  return tf.contrib.training.HParams(
      stem_multiplier=3.0,
...
# label smoothing: 0.1
# clip global norm of all gradients by 10
def mobile_imagenet_config(data_format):
  return tf.contrib.training.HParams(
      stem_multiplier=1.0,
      dense_dropout_keep_prob=0.5,
      num_cells=12,
...
def build_nasnet_mobile(images, num_classes,
  ...
  hparams = (mobile_imagenet_config(data_format=data_format) if config is None
...
def build_nasnet_large(images, num_classes,
  ...
  hparams = (large_imagenet_config(data_format=data_format) if config is None
             else copy.deepcopy(config))
  _update_hparams(hparams, is_training)
